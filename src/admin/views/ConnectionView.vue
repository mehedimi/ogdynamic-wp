    ...
    <div class="ogd:rounded-[20px] ogd:border ogd:border-gray-100 ogd:bg-white ogd:p-6">
      ...
      <div class="ogd:mt-6">
        <FormField label="API key" for-id="ogd-api-key">
          <TextInput id="ogd-api-key" v-model="form.api_key" size="sm" type="password" placeholder="Paste your ogdynamic API key" />
        </FormField>
      </div>

      <div class="ogd:flex ogd:flex-wrap ogd:gap-2.5">
        <button class="ogd:inline-flex ogd:cursor-pointer ogd:items-center ogd:justify-center ogd:gap-2 ogd:rounded-full ogd:border ogd:border-transparent ogd:bg-gray-900 ogd:px-[18px] ogd:py-2.5 ogd:text-[13px] ogd:font-semibold ogd:text-white ogd:disabled:cursor-not-allowed ogd:disabled:opacity-50" type="button" :disabled="api.loading.value" @click="testConnection">
          Test and save connection
        </button>
        <button class="ogd:inline-flex ogd:cursor-pointer ogd:items-center ogd:justify-center ogd:gap-2 ogd:rounded-full ogd:border ogd:border-rose-200 ogd:bg-white ogd:px-[18px] ogd:py-2.5 ogd:text-[13px] ogd:font-semibold ogd:text-rose-500 ogd:disabled:cursor-not-allowed ogd:disabled:opacity-50" type="button" :disabled="api.loading.value" @click="disconnect">
          Disconnect
        </button>
      </div>
